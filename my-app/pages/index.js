...
export default function Home() {
  ...
  // joinedWhitelist keeps track of whether the current user is in the whitelist or not
  ...
  /* 
    addAddressToWhitelist: Adds the current connected address to the whitelis
     */
  ...
  const getNumberOfWhitelisted = async () => {
    try {
      ...
      // have read-onlu access to the Contract
      const whitelistContract = new Contract(
        WHITELIST_CONTRACT_ADDRESS,
        abi,
        provider
  ...
  useEffect(() => {
    ...
    if (!walletConnected) {
      // Assign the Web3Modal calls to the reference object by setting it's 'current' value
